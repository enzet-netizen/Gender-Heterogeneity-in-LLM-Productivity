...
import delimited "panel_gender.csv", clear

ppmlhdfe monthly_productivity post_treated ///
    if gender_final == "male", ///
    absorb(hashed_author cohort_id##month_id) vce(cluster hashed_author)
local b_m  = _b[post_treated]
local se_m = _se[post_treated]

ppmlhdfe monthly_productivity post_treated ///
    if gender_final == "female", ///
    absorb(hashed_author cohort_id##month_id) vce(cluster hashed_author)
...
export delimited "gender_coefficients.csv", replace
di "Saved gender_coefficients.csv"
list
